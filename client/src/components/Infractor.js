...
function BusquedaPatente() {
  ...
  const toggleSeleccion = (infraccion) => {
    ...
    nuevasSeleccionadas[infraccion.infraccion] = !nuevasSeleccionadas[infraccion.infraccion];
    ...
    for (const key in nuevasSeleccionadas) {
      if (nuevasSeleccionadas[key]) {
        nuevoCostoTotal += resultados.infracciones.find(infraccion => infraccion.infraccion === key).costo;
      }
    }
